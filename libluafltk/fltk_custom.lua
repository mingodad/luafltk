...
function post_register_code_hook(package, output)
	output(' Fl::user_data = (void*)tolua_S;')
end

-- flags
local disable_virtual_hooks = false
local enable_pure_virtual = false
...
function classVirtualClass:add(f)
	...
	elseif f.name == 'delete' then
		...
		--print(self.flags.parent_object.lname, f.name)
...
function preprocess(sig)
	...
	--DAD sig = gsub(sig,"([^%w_])void%s*%*","%1_userdata ") -- substitute 'void*'
...
function checkFLTKBaseType(btype, fname)
	if btype:find('Fl_Table') and fname == 'do_callback' then
		return 'Fl_Widget'
	else
		return btype
	end
	...
	for k,v in ipairs(self.methods) do
		...
		if v.f.access ~= 2 and (not v.f.pure_virtual) and v.f.name ~= 'new' and v.f.name ~= 'delete' then
			...
			if rettype == ' void ' then
				output("\t\t".. checkFLTKBaseType(self.btype,v.f.name).."::"..v.f.name..var_list..";")
			else
...
function classVirtualClass:output_method(f, btype)
	...
	if f.pure_virtual then
		...
		if rettype == ' void ' then
			output('\t\t\t',checkFLTKBaseType(btype,f.name),'::',f.name,var_list,';')
		else
