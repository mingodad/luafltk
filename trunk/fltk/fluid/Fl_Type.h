//
// "$Id: Fl_Type.h 7169 2010-02-27 22:38:25Z matt $"
//
...
class Fl_Window_Scroll : public Fl_Window {
    public:
      Fl_Scroll *dsa; //design scrollable area
      void _init() {
          dsa = new Fl_Scroll(x(), y(), w(), h(), 0);
          resizable(dsa);
      }
      Fl_Window_Scroll(int X, int Y, int W, int H, char *L=0) : Fl_Window(X, Y, W, H, L) {_init();}
      Fl_Window_Scroll(int W, int H, char *L=0) : Fl_Window(W, H, L) {_init();}
};
...
//
// End of "$Id: Fl_Type.h 7169 2010-02-27 22:38:25Z matt $".
//
