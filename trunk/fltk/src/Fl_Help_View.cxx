					...
					switch ( buf_tag ) {
						...
						case TAG_TD:
							...
							if ( is_close_tag ) {
								...
								ty = block->y - topline_ - fsize - td_top_gap_;
								tw = block->w - block->x + 7;
								th = block->h + fsize - td_bottom_gap_;
				...
				switch ( buf_tag ) {
					...
					case TAG_UL:
						...
						if ( is_close_tag ) {
							...
							//DAD changing y coordinate when closgin h1 ??
							if ( tolower ( buf[2] ) == 'l' ) {
								yy += fsize + font_increment_;
							}
							...
							switch ( buf_tag ) {
								...
								case TAG_TABLE:
									...
									border_color = get_color ( get_attr ( attrs, "BORDERCOLOR", attr, sizeof ( attr ) ), FL_BLACK );
					...
					case TAG_TD:
						...
						if ( is_close_tag ) {
							...
							xx = blocks_[row].x + 6 + table_offset;
